...
package webdav // import "github.com/lsornana/net/webdav"
...
func (h *Handler) handlePropfind(w http.ResponseWriter, r *http.Request) (status int, err error) {
	...
	walkFn := func(reqPath string, info os.FileInfo, err error) error {
		...
		var pstats []Propstat
		if pf.Propname != nil {
			pnames, err := propnames(ctx, h.FileSystem, h.LockSystem, info)
			if err != nil {
				return err
			}
			...
			pstats, err = allprop(ctx, h.FileSystem, h.LockSystem, info, pf.Prop)
		} else {
			pstats, err = props(ctx, h.FileSystem, h.LockSystem, info, pf.Prop)
		}
		if err != nil {
			return err
		}
